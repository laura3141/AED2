...
public class OrdenacaoInsercao{
    ...
    //Método para ordenar os jogadores por ordem de ano de nascimento, e se for igual por ordem alfabetica
    public static Jogador[] insercao(Jogador jogadores[],int tam){
        for(int i=1;i<tam;i++){
            numeroM++;
            ...
            while(j>=0&&jogadores[j].getAnoNascimento()>=tmp.getAnoNascimento()){
                ...
                if(jogadores[j].getAnoNascimento()==tmp.getAnoNascimento()){
                    int c=0;
                    numeroC++;
                    while(jogadores[j].getNome().charAt(c)==tmp.getNome().charAt(c)){
                        c++;
                    }
                    numeroC++;
                    if(jogadores[j].getNome().charAt(c)>tmp.getNome().charAt(c)){
                        numeroM++;
     ...
     public static Jogador[] insercaoAlfabetica(Jogador jogadores[],int tam){
        for(int i=1;i<tam;i++){
            numeroM++;
            Jogador tmp=jogadores[i];
            int j=i-1;
            ...
            while(j>=0&&jogadores[j].getAnoNascimento()>=tmp.getAnoNascimento()){
                ...
                if(jogadores[j].getAnoNascimento()==tmp.getAnoNascimento()){
                    numeroC++;
                    if(jogadores[j].getNome().charAt(0)>=tmp.getNome().charAt(0))
                    {   numeroC++;
                        if(jogadores[j].getNome().charAt(0)==tmp.getNome().charAt(0)){
                            numeroC++;
                            if(jogadores[j].getNome().charAt(1)>tmp.getNome().charAt(1)){
                                numeroM++;
                                jogadores[j+1]=jogadores[j];
                                j--;
                            }
    ...
    public static void main(String[] args) {
       ...
       //Chamada do metodo para ordenaçao do subarray
        ...
        jogadoresO=insercao(jogadoresSub, c);
