...
//funcao para ordenar os jogadores com base no ano de nascimento
void bolha(Jogador jogadores[],int tam){
    for(int c=0;c<tam;c++){
        for(int i=0;i<tam-1-c;i++){
            numeroC++;
            if((jogadores[i+1].anoNascimento<jogadores[i].anoNascimento)){
                numeroM++;
                ...
                if((jogadores[i+1].anoNascimento==jogadores[i].anoNascimento)){
                    numeroC++;
                    if(strcmp(jogadores[i+1].nome,jogadores[i].nome)<0){
                        numeroM++;
                        Jogador aux=jogadores[i];
                        jogadores[i]=jogadores[i+1];
                        jogadores[i+1]=aux;
...
void criaLog(){
    FILE *arq=fopen("808756_bolha.txt","w");
